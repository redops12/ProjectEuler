# Solutions
add_executable( sol7 solutions/7.cpp )
target_include_directories( sol7 PUBLIC
    "${PROJECT_BINARY_DIR}"
    "${PROJECT_SOURCE_DIR}/lib/include"
)
target_link_libraries( sol7 PRIVATE lib )
add_test(NAME test7 COMMAND sol7)

add_executable( sol8 solutions/8.cpp )

add_executable( sol10 solutions/10.cpp )
target_link_libraries( sol10 PRIVATE lib )
add_test(NAME test10 COMMAND sol10)


add_executable( sol11 solutions/11.cpp )

add_executable( sol12 solutions/12.cpp )

add_executable( sol13 solutions/13.cpp )

add_executable( sol14 solutions/14.cpp )

add_executable( sol15 solutions/15.cpp )
target_link_libraries( sol15 PRIVATE lib )
add_test(NAME test15 COMMAND sol15)

add_executable( sol16 solutions/16.cpp )

add_executable( sol17 solutions/17.cpp )

add_executable( sol18 solutions/18.cpp )

add_executable( sol19 solutions/19.cpp )

add_executable( sol20 solutions/20.cpp )
target_link_libraries( sol20 PRIVATE lib )
add_test(NAME test20 COMMAND sol20)

add_executable( sol21 solutions/21.cpp )

add_executable( sol22 solutions/22.cpp )

add_executable( sol23 solutions/23.cpp )

add_executable( sol25 solutions/25.cpp )

add_executable( sol26 solutions/26.cpp )
target_link_libraries( sol26 PRIVATE lib )
add_test(NAME test26 COMMAND sol26)

add_executable( sol27 solutions/27.cpp )
target_link_libraries( sol27 PRIVATE lib )
add_test(NAME test27 COMMAND sol27)

add_executable( sol28 solutions/28.cpp )

add_executable( sol29 solutions/29.cpp )
target_link_libraries( sol29 PRIVATE lib )
add_test(NAME test29 COMMAND sol29)

add_executable( sol30 solutions/30.cpp )
target_link_libraries( sol30 PRIVATE lib )
add_test(NAME test30 COMMAND sol30)

add_executable( sol31 solutions/31.cpp )

add_executable( sol32 solutions/32.cpp )

add_executable( sol33 solutions/33.cpp )
target_link_libraries( sol33 PRIVATE lib )
add_test(NAME test33 COMMAND sol33)

add_executable( sol34 solutions/34.cpp )
target_link_libraries( sol34 PRIVATE lib )
add_test(NAME test34 COMMAND sol34)

add_executable( sol35 solutions/35.cpp )
target_link_libraries( sol35 PRIVATE lib )
add_test(NAME test35 COMMAND sol35)

add_executable( sol36 solutions/36.cpp )
target_link_libraries( sol36 PRIVATE lib )
add_test(NAME test36 COMMAND sol36)

add_executable( sol37 solutions/37.cpp )
target_link_libraries( sol37 PRIVATE lib )
add_test(NAME test37 COMMAND sol37)

add_executable( sol38 solutions/38.cpp )
target_link_libraries( sol38 PRIVATE lib )
add_test(NAME test38 COMMAND sol38)

add_executable( sol39 solutions/39.cpp )

add_executable( sol40 solutions/40.cpp )

add_executable( sol41 solutions/41.cpp )
target_link_libraries( sol41 PRIVATE lib )
add_test(NAME test41 COMMAND sol41)

add_executable( sol42 solutions/42.cpp )

add_executable( sol43 solutions/43.cpp )
target_link_libraries( sol43 PRIVATE lib )
add_test(NAME test43 COMMAND sol43)

add_executable( sol44 solutions/44.cpp )

add_executable( sol45 solutions/45.cpp )

add_executable( sol46 solutions/46.cpp )
target_link_libraries( sol46 PRIVATE lib )
add_test(NAME test46 COMMAND sol46)

add_executable( sol47 solutions/47.cpp )
target_link_libraries( sol47 PRIVATE lib )
add_test(NAME test47 COMMAND sol47)

add_executable( sol48 solutions/48.cpp )

add_executable( sol49 solutions/49.cpp )
target_link_libraries( sol49 PRIVATE lib )
add_test(NAME test49 COMMAND sol49)

add_executable( sol50 solutions/50.cpp )
target_link_libraries( sol50 PRIVATE lib )
add_test(NAME test50 COMMAND sol50)

add_executable( sol51 solutions/51.cpp )
target_link_libraries( sol51 PRIVATE lib )
add_test(NAME test51 COMMAND sol51)

add_executable( sol52 solutions/52.cpp )
target_link_libraries( sol52 PRIVATE lib )
add_test(NAME test52 COMMAND sol52)

add_executable( sol53 solutions/53.cpp )
target_link_libraries( sol53 PRIVATE lib )
add_test(NAME test53 COMMAND sol53)

add_executable( sol54 solutions/54.cpp )

add_executable( sol55 solutions/55.cpp )
target_link_libraries( sol55 PRIVATE lib )
add_test(NAME test55 COMMAND sol55)

add_executable( sol56 solutions/56.cpp )
target_link_libraries( sol56 PRIVATE lib )
add_test(NAME test56 COMMAND sol56)

add_executable( sol57 solutions/57.cpp )
target_link_libraries( sol57 PRIVATE lib )
add_test(NAME test57 COMMAND sol57)

add_executable( sol58 solutions/58.cpp )
target_link_libraries( sol58 PRIVATE lib )
add_test(NAME test58 COMMAND sol58)

add_executable( sol59 solutions/59.cpp )

add_executable( sol60 solutions/60.cpp )
target_link_libraries( sol60 PRIVATE lib )
add_test(NAME test60 COMMAND sol60)

add_executable( sol61 solutions/61.cpp )

add_executable( sol62 solutions/62.cpp )
target_link_libraries( sol62 PRIVATE lib )
add_test(NAME test62 COMMAND sol62)

add_executable( sol63 solutions/63.cpp )
target_link_libraries( sol63 PRIVATE lib )
add_test(NAME test63 COMMAND sol63)

add_executable( sol64 solutions/64.cpp )
target_link_libraries( sol64 PRIVATE lib )
add_test(NAME test64 COMMAND sol64)

add_executable( sol65 solutions/65.cpp )
target_link_libraries( sol65 PRIVATE lib )
add_test(NAME test65 COMMAND sol65)

add_executable( sol66 solutions/66.cpp )
target_link_libraries( sol66 PRIVATE lib )
add_test(NAME test66 COMMAND sol66)

add_executable( sol67 solutions/67.cpp )

add_executable( sol68 solutions/68.cpp )
target_link_libraries( sol68 PRIVATE lib )
add_test(NAME test68 COMMAND sol68)

add_executable( sol70 solutions/70.cpp )
target_link_libraries( sol70 PRIVATE lib )
add_test(NAME test70 COMMAND sol70)

add_executable( sol71 solutions/71.cpp )

add_executable( sol72 solutions/72.cpp )
target_link_libraries( sol72 PRIVATE lib )
add_test(NAME test72 COMMAND sol72)

add_executable( sol73 solutions/73.cpp )
target_link_libraries( sol73 PRIVATE lib )
add_test(NAME test73 COMMAND sol73)

add_executable( sol74 solutions/74.cpp )
target_link_libraries( sol74 PRIVATE lib )
add_test(NAME test74 COMMAND sol74)

add_executable( sol75 solutions/75.cpp )
target_link_libraries( sol75 PRIVATE lib )
add_test(NAME test75 COMMAND sol75)

add_executable( sol76 solutions/76.cpp )
target_link_libraries( sol76 PRIVATE lib )
add_test(NAME test76 COMMAND sol76)

add_executable( sol78 solutions/78.cpp )
target_link_libraries( sol78 PRIVATE lib )
add_test(NAME test78 COMMAND sol78)

add_executable( sol80 solutions/80.cpp )
target_link_libraries( sol80 PRIVATE lib )
add_test(NAME test80 COMMAND sol80)

add_executable( sol77 solutions/77.cpp )
target_link_libraries( sol77 PRIVATE lib )
add_test(NAME test77 COMMAND sol77)

add_executable( sol81 solutions/81.cpp )

add_executable( sol82 solutions/82.cpp )
target_link_libraries( sol82 PRIVATE lib )
add_test(NAME test82 COMMAND sol82)

add_executable( sol83 solutions/83.cpp )
target_link_libraries( sol83 PRIVATE lib )
add_test(NAME test83 COMMAND sol83)

add_executable( sol84 solutions/84.cpp )

add_executable( sol85 solutions/85.cpp )

add_executable( sol86 solutions/86.cpp )
target_link_libraries( sol86 PRIVATE lib )
add_test(NAME test86 COMMAND sol86)

add_executable( sol87 solutions/87.cpp )
target_link_libraries( sol87 PRIVATE lib )
add_test(NAME test87 COMMAND sol87)

add_executable( sol88 solutions/88.cpp )
target_link_libraries( sol88 PRIVATE lib )
add_test(NAME test88 COMMAND sol88)

add_executable( sol89 solutions/89.cpp )
target_link_libraries( sol89 PRIVATE lib )
add_test(NAME test89 COMMAND sol89)

add_executable( sol90 solutions/90.cpp )

add_executable( sol91 solutions/91.cpp )

add_executable( sol92 solutions/92.cpp )
target_link_libraries( sol92 PRIVATE lib )
add_test(NAME test92 COMMAND sol92)

add_executable( sol93 solutions/93.cpp )
target_link_libraries( sol93 PRIVATE lib )
add_test(NAME test93 COMMAND sol93)

add_executable( sol94 solutions/94.cpp )
target_link_libraries( sol94 PRIVATE lib )
add_test(NAME test94 COMMAND sol94)

add_executable( sol95 solutions/95.cpp )
target_link_libraries( sol95 PRIVATE lib )
add_test(NAME test95 COMMAND sol95)

add_executable( sol96 solutions/96.cpp )

add_executable( sol97 solutions/97.cpp )

add_executable( sol98 solutions/98.cpp )

add_executable( sol99 solutions/99.cpp )

add_executable( sol101 solutions/101.cpp )
target_link_libraries( sol101 PRIVATE lib )
add_test(NAME test101 COMMAND sol101)

add_executable( sol102 solutions/102.cpp )
target_link_libraries( sol102 PRIVATE lib )
add_test(NAME test102 COMMAND sol102)

add_executable( sol104 solutions/104.cpp )
target_link_libraries( sol104 PRIVATE lib )
add_test(NAME test104 COMMAND sol104)

add_executable( sol193 solutions/193.cpp )
target_link_libraries( sol193 PRIVATE lib )
add_test(NAME test193 COMMAND sol193)

add_executable( sol206 solutions/206.cpp )

add_executable( sol216 solutions/216.cpp )
target_link_libraries( sol216 PRIVATE lib )
add_test(NAME test216 COMMAND sol216)

add_executable( sol700 solutions/700.cpp )
target_link_libraries( sol700 PRIVATE lib )
add_test(NAME test700 COMMAND sol700)

add_executable( sol926 solutions/926.cpp )
target_link_libraries( sol926 PRIVATE lib )
add_test(NAME test926 COMMAND sol926)

add_executable( soladvent2 solutions/advent2.cpp )
target_link_libraries( soladvent2 PRIVATE lib )
add_test(NAME testadvent2 COMMAND soladvent2)

add_executable( soladvent3 solutions/advent3.cpp )
